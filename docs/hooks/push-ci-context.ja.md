...
`git push` または `gh pr create` の成功後、残りの作業を先に終え、それから `hhx wait-ci --progress` で PR の check の終了を待ち、結果を 1 回だけ報告するようエージェントに案内します。短い間隔で繰り返し確認することや、CI を見ずにターンを終えることを避けます。
