...
import (
	...
	"github.com/HappyOnigiri/hhx/tools/internal/gotest"
)

func resolveFailedDeclarations(
	ctx context.Context, cfg config, failed map[string][]string,
) (map[string]map[string]declaration, []string) {
...
func retryFailures(
	ctx context.Context, cfg config, initial testResult, failed map[string][]string,
	resolved map[string]map[string]declaration, man *manifest, output io.Writer,
) string {
	status := "failed"
	...
	for packageName, names := range failed {
		...
		shuffle := initial.ShuffleByPackage[packageName]
		command, coverage, err := retryCommand(cfg, packageName, roots, shuffle, len(man.Retries))
		retry := retryRecord{
			Package:     packageName,
			Functions:   roots,
			Command:     command,
		...
		label := fmt.Sprintf("retry-%02d-%s", len(man.Retries)+1, safeName(packageName))
		result, executeErr := executeRun(ctx, cfg, command, label, output)
		...
		retry.JSONL = label + ".jsonl"
		retry.Log = label + ".log"
		retry.Stderr = label + ".stderr"
