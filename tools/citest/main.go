...
func run(ctx context.Context, cfg config, output io.Writer) (int, error) {
	...
	initialResult, err := executeRun(ctx, cfg, initialCommand, "initial", output)
	if err != nil {
		return 1, err
	}
	initialRecord := runRecordFromResult(initialResult, initialCommand, "initial", cfg.CoverageProfile)
	man := newManifest(cfg, initialRecord, started)
...
// supportedProfiles は CI が報告を上げるテストのジョブである。
// report-flaky-tests.cjs の PROFILE_CONTRACTS と ci.yml の matrix の citest-profile に合わせる。
...
func newManifest(cfg config, initial runRecord, started time.Time) manifest {
	...
	for _, key := range []string{"CI", "GOMAXPROCS", "GODEBUG"} {
