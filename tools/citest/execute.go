...
func executeRun(ctx context.Context, cfg config, command []string, label string, output io.Writer) (testResult, error) {
	started := now()
	...
	result := testResult{
		Tests:            make(map[gotest.TestID][]testEvent),
		ShuffleByPackage: make(map[string]string),
		StartedAt:        started,
	}
